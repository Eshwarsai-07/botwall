...
const SECRET = process.env.BOTWALL_AUTH_SECRET
  || process.env.BOTWALL_CHALLENGE_SECRET
  || "dev-secret-change-me";

const DOMAIN = process.env.BOTWALL_AUTH_DOMAIN || "botwall.dev";
...
export function requireWalletSession(req, res, next) {
  ...
  } else if (typeof req.headers["x-botwall-session"] === "string") {
    token = req.headers["x-botwall-session"].trim();
  }
