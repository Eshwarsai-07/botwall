# Botwall — HTTP 402 Paywall for AI Agents

[![npm](https://img.shields.io/npm/v/botwall-sdk?label=botwall-sdk&color=f59e0b)](https://www.npmjs.com/package/botwall-sdk)
[![npm](https://img.shields.io/npm/v/botwall-agent-sdk?label=botwall-agent-sdk&color=f59e0b)](https://www.npmjs.com/package/botwall-agent-sdk)
[![License: MIT](https://img.shields.io/badge/license-MIT-green)](LICENSE)
[![Live Demo](https://img.shields.io/badge/live%20demo-botwall--ebon.vercel.app-blueviolet)](https://botwall-ebon.vercel.app/)

> **robots.txt was a suggestion. This isn't.**

Botwall makes AI agent content access **enforceable at the protocol layer**. Publishers drop in two lines of SDK to gate any route and receive USDC micropayments directly in their Solana wallet. AI agents pay automatically, or they don't get in.
...
| Resource | URL |
|---|---|
| 🌐 Landing page | [botwall-ebon.vercel.app](https://botwall-ebon.vercel.app/) |
| ⚙️ Backend API | [botwall-production.up.railway.app](https://botwall-production.up.railway.app) |
| 📦 Publisher SDK (npm) | [npmjs.com/package/botwall-sdk](https://www.npmjs.com/package/botwall-sdk) |
| 🤖 Agent SDK (npm) | [npmjs.com/package/botwall-agent-sdk](https://www.npmjs.com/package/botwall-agent-sdk) |
| 📖 Publisher docs | [docs/PUBLISHER.md](docs/PUBLISHER.md) |
...
Botwall attaches a **price tag** to bot access using HTTP 402 Payment Required. The server issues a signed payment challenge; the agent submits a USDC transfer on Solana and retries with a payment header. Content unlocks in ~400ms. USDC lands directly in the publisher's wallet — no platform cut, no intermediary.
...
AI Agent                    Publisher Server              Solana
   ...
   │   x-botwall-challenge: ...   │◀── confirmed ────────────│
   │                              │                          │
   │◀── HTTP 200 + content ───────│                          │
```

1. AI bot hits a protected route → server returns HTTP 402 with an x402 payment envelope
2. Agent SDK reads the envelope, submits a USDC SPL transfer on Solana, retries with `X-PAYMENT` and `x-botwall-challenge` headers
...
curl https://botwall-production.up.railway.app/health

# 2. Hit it as an AI bot — gets HTTP 402
curl -A "GPTBot/1.0" https://botwall-production.up.railway.app/articles/test
...
npm install botwall-agent-sdk @solana/web3.js @solana/spl-token @x402-solana/core
```
...
Full reference: [docs/PUBLISHER.md](docs/PUBLISHER.md)
...
```js
import { createAgentPaywallClient, fromKeypairFile } from "botwall-agent-sdk";
...
```bash
git clone https://github.com/Eshwarsai-07/botwall.git
cd botwall
npm install
```
...
# Bot request against live server
curl -A "GPTBot/1.0" https://botwall-production.up.railway.app/articles/test
```
...
BASE_URL=https://botwall-production.up.railway.app npm run test:e2e
...
4. Retries with `X-PAYMENT` + `x-botwall-challenge` headers → asserts `HTTP 200`
...
│   │           ├── fastify.js
│   │           └── cloudflare.js
│   └── botwall-agent-sdk/     # botwall-agent-sdk (npm)
│       └── src/
...
**Base URL:** `https://botwall-production.up.railway.app`
...
- [Landing page](https://botwall-ebon.vercel.app/)
