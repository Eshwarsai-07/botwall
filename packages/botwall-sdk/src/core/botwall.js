/**
 * Framework-agnostic botwall orchestrator.
 *
 * Adapters convert framework-specific request/response into the normalized
 * interface here, call `runBotwall(...)`, and translate the verdict back.
 *
 * Verdict shapes:
 *   { kind: "passthrough" }
...
export async function runBotwall({
  client,
  config,
  ...
  const challengeToken = getHeader(headers, "x-botwall-challenge");
...
function failOpen(config, err, stage) {
  ...
  return {
    ...
    status: 503,
    headers: HEADERS_JSON,
    body: { status: "error", stage, error: err.message || "Botwall unavailable" },
  };
}
