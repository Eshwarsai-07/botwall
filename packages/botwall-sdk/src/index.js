/**
 * botwall-sdk — drop-in AI bot paywall.
 *
 * Quick start (Express):
 *   import { createBotwall } from "botwall-sdk";
 *   import { expressMiddleware } from "botwall-sdk/express";
 *
 *   const botwall = createBotwall({
 *     walletAddress: process.env.SOLANA_WALLET_ADDRESS,
 *     network: "devnet",
 *   });
 *   app.use("/articles", expressMiddleware(botwall));
 *
...
import { BotwallClient } from "./core/client.js";
import { runBotwall } from "./core/botwall.js";
import { detectBot } from "./core/botDetector.js";

/**
 * @typedef {object} BotwallConfig
...
/**
 * Create a botwall instance from config.
 * The returned object is consumed by framework adapters.
 *
 * @param {BotwallConfig} config
 */
export function createBotwall(config) {
  if (!config || !config.walletAddress) {
    throw new Error(
      "createBotwall requires { walletAddress }. Pass your Solana wallet address — that's where USDC payments will land.",
    );
  }
  if (!isLikelySolanaAddress(config.walletAddress)) {
    throw new Error(
      `createBotwall: walletAddress "${config.walletAddress}" does not look like a valid Solana address (base58, 32-44 chars).`,
    );
  }
  ...
  return {
     ...
     */
    run: (request) => runBotwall({ client, config: resolvedConfig, request }),
  };
}
