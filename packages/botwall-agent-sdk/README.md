...
[![npm version](https://img.shields.io/npm/v/botwall-agent-sdk?color=f59e0b)](https://www.npmjs.com/package/botwall-agent-sdk)
...
> If you publish content, use `botwall-sdk` to **gate** it.
> If you build agents, use `botwall-agent-sdk` to **pay** for it.

**Links:** [npm](https://www.npmjs.com/package/botwall-agent-sdk) · [Live demo](https://botwall-ebon.vercel.app/) · [Full agent guide](../../docs/AGENT.md) · [Backend API](https://botwall-production.up.railway.app)
...
5. retries the original request with `X-PAYMENT` and `x-botwall-challenge`
...
// Fetch a Botwall-protected route — payment is handled automatically
const res = await client.fetch("https://botwall-production.up.railway.app/articles/test");
...
> **Local testing:** Set `CONSUMER_URL=http://localhost:4010` (or your local server address)
> when running examples against a local publisher server.
...
violation throws a typed error from `botwall-agent-sdk/errors`:
...
```js
import { PaymentRefusedError } from "botwall-agent-sdk";
...
```js
import { paywallFetchTool } from "botwall-agent-sdk/langchain";
...
| Manually echo `x-botwall-challenge`  | Automatic                           |
