/**
 * Agent botwall client.
 *
 *   const client = createAgentBotwallClient({ network, signer, ...guards });
 *   const res = await client.fetch(url, init);
 *
 * On a `200` (or any non-402 response), `client.fetch` is a passthrough
 * over `globalThis.fetch`. On a `402` from an `botwall-sdk`-compatible
 * server, it:
 *
 ...
 *      as a non-enumerable `botwallPayment` property
 *
...
const DEFAULT_USER_AGENT = "botwall-agent-sdk/0.1";
const PAYMENT_HEADER = "X-PAYMENT";
const CHALLENGE_HEADER = "x-botwall-challenge";
...
function attachPayment(response, payment) {
  Object.defineProperty(response, "botwallPayment", {
    value: payment,
    enumerable: false,
    configurable: false,
    writable: false,
...
/**
 * @typedef {object} AgentBotwallConfig
 ...
 * @property {string} [userAgent]                           - User-Agent for requests (defaults to botwall-agent-sdk)
...
/**
 * @param {AgentBotwallConfig} input
 */
export function createAgentBotwallClient(input) {
  if (!input || typeof input !== "object") {
    throw new Error("createAgentBotwallClient: config object is required");
  }
   ...
   *
   * Returns a normal `Response`. If a payment was made, `response.botwallPayment`
