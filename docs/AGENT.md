# Agent Guide — Paying HTTP 402 Paywalls with `botwall-agent-sdk`
...
```js
import { fromSecretKeyArray } from "botwall-agent-sdk";
...
```js
import { fromSecretKeyBase58 } from "botwall-agent-sdk";
...
```js
import { fromKeypair } from "botwall-agent-sdk";
...
| `userAgent` | — | `botwall-agent-sdk/0.1` | User-Agent header sent with all requests. |
...
try {
  ...
  if (err instanceof PaymentRefusedError) {
    ...
    // May be a non-Botwall paywall or a misconfigured server.
