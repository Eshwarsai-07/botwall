# Publisher Guide — Protecting Your Content with `botwall-sdk`
...
You can also use the `x-botwall-session` header instead of `Authorization: Bearer`:

```bash
curl https://your-backend.com/v1/dashboard \
  -H "x-botwall-session: <session-token>"
```
...
BOTWALL_AUTH_DOMAIN=yourdomain.com             # shown in the Sign-In message
...
- [ ] Set `BOTWALL_CHALLENGE_SECRET` and `BOTWALL_AUTH_SECRET` to fresh 32-byte secrets
- [ ] Set `BOTWALL_AUTH_DOMAIN` to your actual domain
