# Botwall — Design Notes

## Chosen Name: Botwall

**Rationale**: A botwall is real enforcement infrastructure — not a polite notice. The firewall metaphor maps exactly to the product's core value proposition: you pay to pass, or you don't pass. Unlike robots.txt (a text file any agent can ignore), a botwall is physical. The name earns its tagline: *"robots.txt was a suggestion. Botwall isn't."*
...
- **x402** — leverages the HTTP standard, but risks confusion with the existing `@x402-solana` npm package family that Botwall depends on.
...
**Why**: Botwall is developer infrastructure. It uses blockchain incidentally (because the economics demand it), but the primary audience is engineers and publishers who care about the HTTP protocol, the npm install, and the Solana wallet flow — not token aesthetics. The terminal aesthetic matches that context and sets accurate expectations.

**Rejected alternatives**:
- **Web3 gradient-glow (Phantom)**: Would make Botwall look like a DeFi product. It isn't.
...
**Publisher SDK (botwall-sdk v0.2.0)**:
- Bot detection: reads from `packages/botwall-sdk/src/core/botDetector.js`
...
**Agent SDK (botwall-agent-sdk v0.1.0)**:
