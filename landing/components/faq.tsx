...
const FAQS = [
  ...
  {
    q: "Where does the money go? Does Botwall take a cut?",
    a: "Payments flow directly from the agent's Solana wallet to your wallet ATA on-chain. Botwall's server never holds funds, has no custody of keys, and takes no percentage of payments. The only fees are Solana network fees (~$0.00025 per transaction). The plan fee covers hosted infrastructure and support, not a slice of your revenue.",
  },
  {
  ...
  },
  {
    ...
    a: "Install botwall-agent-sdk, provide a Solana keypair (or custom signer), and replace fetch() with client.fetch(). The SDK handles the entire payment flow — parsing the 402, building and submitting the USDC transfer, retrying the request with the X-PAYMENT header. For LangChain, use paywallFetchTool(client) to expose it as a tool. That's the full integration.",
  },
];
