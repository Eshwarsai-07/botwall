...
export const metadata = {
  title: "Publisher SDK — Botwall Docs",
    ...
    <DocLayout sdk="publisher" sections={SECTIONS}>
      ...
      <div className="space-y-0">
        ...
        <DocSection id="overview" title="Overview">
          <DocP>
            Botwall intercepts HTTP requests at the middleware layer. When it detects an AI bot,
        ...
        <DocSection id="install" title="Installation">
          <DocCode lang="bash">{`npm install botwall-sdk`}</DocCode>
          <DocP>
...
app.get("/articles/:slug", async (req, reply) => {
  return { content: "Your article..." };
          ...
          <DocSubSection id="cloudflare" title="Cloudflare Workers">
            <DocCode lang="js">{`import { createPaywall } from "botwall-sdk";
      ...
      ip:       d.ip,
        ...
        <DocSection id="dashboard" title="Dashboard & Auth">
          ...
          <DocSubSection title="Step 1 — Request a nonce">
            <DocCode lang="bash">{`curl -X POST https://botwall-production-f453.up.railway.app/v1/auth/nonce \\
...
# {
#   "token": "<opaque-token>",
#   "message": "botwall.dev wants you to sign in...",
...
const signatureBase58 = bs58.encode(signature);`}</DocCode>
          ...
          <DocSubSection title="Step 3 — Exchange for a session (valid 24h)">
            <DocCode lang="bash">{`curl -X POST https://botwall-production-f453.up.railway.app/v1/auth/verify \\
  -H "Content-Type: application/json" \\
  -d '{
...
# Response: { "session": "<token>", "expiresAt": "..." }`}</DocCode>
          ...
          <DocSubSection title="Step 4 — Fetch analytics">
            <DocCode lang="bash">{`curl https://botwall-production-f453.up.railway.app/v1/dashboard \\
...
curl "https://botwall-production-f453.up.railway.app/v1/wallet/treasury?walletAddress=YourWallet&network=mainnet-beta"
...
BOTWALL_CHALLENGE_SECRET=<32-byte-hex>
BOTWALL_AUTH_SECRET=<32-byte-hex>
BOTWALL_AUTH_DOMAIN=yourdomain.com
...
FACILITATOR_FEE_PAYER_SECRET_KEY=[...keypair-json-array...]`}</DocCode>
          <DocCallout type="warning">
            ...
            <DocBadge>BOTWALL_CHALLENGE_SECRET</DocBadge> and{" "}
            <DocBadge>BOTWALL_AUTH_SECRET</DocBadge> before going to production.
          </DocCallout>
        </DocSection>
      </div>
    </DocLayout>
